...
class disparityregression(nn.Module):
    ...
    def forward(self, x): # x = torch.Size([1, 64, 480, 640])
        disp = self.disp.repeat(x.size()[0],1,x.size()[2],x.size()[3]) # torch.Size([1, 64, 480, 640])
...
class feature_extraction(nn.Module):
    ...
    def forward(self, x):                 # torch.Size([1, 3, 480, 640])
        output      = self.firstconv(x)   # torch.Size([1, 32, 240, 320])
